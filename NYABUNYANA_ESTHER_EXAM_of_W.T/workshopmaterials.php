...
<head>
  ...
  <title>workshopmaterials Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    <h1><u>Workshopmaterials Form</u></h1>

<form method="post" onsubmit="return confirmInsert();">

    <label for="WorkshopMaterialID">WorkshopMaterialID:</label>
    ...
    <label for="MaterialDescription">MaterialDescription:</label>
    <input type="TEXT" id="booking_time" name="booking_time" required><br><br>

    <label for="MaterialLink">MaterialLink:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO workshopmaterials(WorkshopMaterialID, WorkshopID, MaterialName,MaterialDescription,MaterialLink) VALUES (?, ?, ?, ?,?)");
    $stmt->bind_param("sssss", $WorkshopMaterialID, $WorkshopID, $MaterialName, $MaterialDescription,$MaterialLink);
    // Set parameters and execute
    $WorkshopMaterialID = $_POST['book_id'];
    $WorkshopID = $_POST['ride_id'];
    $MaterialName = $_POST['passenger_id'];
    $MaterialDescription = $_POST['booking_time'];
    $MaterialLink = $_POST['bookingtime'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Workshopmaterials Details</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Workshopmaterials Table</h2></center>
    <table border="3">
        <tr>
            <th>WorkshopMaterialID</th>
            <th>WorkshopID</th>
            <th>MaterialName</th>
...
// Prepare SQL query to retrieve all workshopmaterials
$sql = "SELECT * FROM workshopmaterials";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $WorkshopMaterialID = $row['WorkshopMaterialID']; // Fetch the WorkshopMaterialID
        echo "<tr>
            <td>" . $row['WorkshopMaterialID'] . "</td>
            <td>" . $row['WorkshopID'] . "</td>
            <td>" . $row['MaterialName'] . "</td>
            <td>" . $row['MaterialDescription'] . "</td>
            <td>" . $row['MaterialLink'] . "</td>
            <td><a style='padding:4px' href='delete_workshopmaterials.php?WorkshopMaterialID=$WorkshopMaterialID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_workshopmaterials.php?WorkshopMaterialID=$WorkshopMaterialID'>Update</a></td> 
        </tr>";
    }
