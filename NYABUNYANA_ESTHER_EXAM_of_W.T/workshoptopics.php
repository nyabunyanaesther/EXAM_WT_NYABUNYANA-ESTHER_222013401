...
<head>
  ...
  <title>Workshoptopics Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    <h1><u>Workshoptopics Form</u></h1>

<form method="post" onsubmit="return confirmInsert();">

    <label for="WorkshopTopicID">WorkshopTopicID:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO workshoptopics(WorkshopTopicID, WorkshopID,TopicID) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $WorkshopTopicID, $WorkshopID,$TopicID);
    // Set parameters and execute
    $WorkshopTopicID = $_POST['book_id'];
    $WorkshopID = $_POST['ride_id'];
    $TopicID = $_POST['passenger_id'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>workshoptopics Details</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Workshoptopics Table</h2></center>
    <table border="3">
        <tr>
            <th>WorkshopTopicID</th>
            <th>WorkshopID</th>
            <th>TopicID</th>
            <th>Delete</th>
            <th>Update</th>
...
// Prepare SQL query to retrieve all workshoptopics
$sql = "SELECT * FROM workshoptopics";
$result = $connection->query($sql);

// Check if there are any workshoptopics
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $WorkshopTopicID = $row['WorkshopTopicID']; // Fetch the WorkshopTopicID
        echo "<tr>
            <td>" . $row['WorkshopTopicID'] . "</td>
            <td>" . $row['WorkshopID'] . "</td>
            <td>" . $row['TopicID'] . "</td>
            <td><a style='padding:4px' href='delete_workshoptopics.php?WorkshopTopicID=$WorkshopTopicID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_workshoptopics.php?WorkshopTopicID=$WorkshopTopicID'>Update</a></td> 
        </tr>";
    }
