...
<head>
  ...
  <title>diversityandinclusionresources Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
    <h1><u>Diversityandinclusionresources Form</u></h1>
...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="UploadedBy">UploadedBy:</label>
     ...
     <input type="date" id="bookintime" name="bookintime" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO diversityandinclusionresources(ResourceID, Title, Description, Link, UploadedBy,UploadDate) VALUES (?, ?, ?, ?,?, ?)");
    $stmt->bind_param("ssssss", $ResourceID, $Title, $Description, $Link, $UploadedBy,$UploadDate);
    // Set parameters and execute
    $ResourceID = $_POST['book_id'];
    $Title = $_POST['ride_id'];
    $Description = $_POST['passenger_id'];
    $Link = $_POST['booking_time'];
    $UploadedBy = $_POST['bookingtime'];
    $UploadDate = $_POST['bookintime'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Diversityandinclusionresources Details</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Diversityandinclusionresources Table</h2></center>
    <table border="3">
        <tr>
            <th>ResourceID</th>
            <th>Title</th>
...
// Prepare SQL query to retrieve all diversityandinclusionresources
$sql = "SELECT * FROM diversityandinclusionresources";
$result = $connection->query($sql);

// Check if there are any diversityandinclusionresources
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_diversityandinclusionresources.php?ResourceID=$ResourceID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_diversityandinclusionresources.php?ResourceID=$ResourceID'>Update</a></td> 
        </tr>";
    }
